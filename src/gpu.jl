...
$(SIGNATURES)
"""
function collect_rt_gpu!(cfg::Config, g::AbstractGeometry, irt, I, k, x, idx, theta_range)
    ...
    for j2 in 1:(cfg.Nϕ)
        for j1 in theta_range
            ...
            if t <= cfg.τ₀
                ...
                CUDA.@atomic irt[1, j1, j2] += I′′[1]
                CUDA.@atomic irt[2, j1, j2] += I′′[2]
                CUDA.@atomic irt[3, j1, j2] += I′′[3]
                CUDA.@atomic irt[4, j1, j2] += I′′[4]
                acc += I′′[1]
            end
        end
    end
...
$(SIGNATURES)
"""
function collect_cb_gpu!(cfg::Config, g::AbstractGeometry, Icb, I, k, x, idx, I₀, xpath,
                         kpath,
                         idxpath, nid, scattered_times)
    xa = x
    ka = k
    idxa = idx
    ...
    if scattered_times > 1 # Only consider CB effects when scattered more than once
        ...
        Ei₁, Ei₂ = E_from_I(I₀)
        Ea₁, Ea₂, enorma = forward_E(sca, Ei₁, Ei₂,
                                     view(kpath, :, nid), scattered_times, cosθ₀, sinθ₀,
                                     cosϕ₀, sinϕ₀)
        Eb₁, Eb₂, enormb = backward_E(sca, Ei₁, Ei₂, view(kpath, :, nid), scattered_times,
                                      cosθ₀, sinθ₀,
                                      cosϕ₀, sinϕ₀)
        ...
        for j2 in 1:(cfg.Nϕb)
            for j1 in 1:(cfg.Nθb)
                ...
                if τa <= cfg.τ₀
                    ...
                    CUDA.@atomic Icb[1, j1, j2] += I1[1]
                    CUDA.@atomic Icb[2, j1, j2] += I1[2]
                    CUDA.@atomic Icb[3, j1, j2] += I1[3]
                    CUDA.@atomic Icb[4, j1, j2] += I1[4]
                end
            end
        end
    else
...
function gpu_kernel(cfg, g, I₀, k₀, Eₕ₀, Eᵥ₀, kpath, xpath, idxpath, A, Irt, Icb)
    tid = threadIdx().x
    bid = blockIdx().x
